...
class SessionKeyGen(threading.Thread):
    ...
    def run(self):
        while True:
            ...
            session_key_cipher = AES.new(session_key, AES.MODE_EAX)
            session_cipher[self.username] = session_key_cipher
            encryption_cipher = PKCS1_OAEP.new(RSA.import_key(clients[self.client_socket].public_key))
            message_hash = SHA256.new(session_key)
            signature = pkcs1_15.new(RSA.import_key(server_private_key)).sign(message_hash)
            encrypted_session_key = encryption_cipher.encrypt(session_key)
            negotiate = "---BEGIN SESSION KEY---".encode() + encrypted_session_key + b"~SIGNATURE~" + signature + "---END SESSION KEY---".encode()
            try:
                ...
                print("Session Key Generator for \"" + self.username + "\" has been disabled.")
                break
    ...
    while True:
        ...
        for notified_socket in read_sockets:
            if notified_socket == server_socket:
                ...
                client_public_key = confirm.partition("~PK~")[2].strip()
                confirm = confirm.partition("~PK~")[0]
                relogin_flag = False
                if confirm == "EXISTS":
                    success_message = "Successfully logged in!"
                    response = success_message + " ~PK~" + str(server_public_key)
                    client_socket.send(response.encode())
                    relogin_flag = True
                else:
                    success_message = "Username set successfully!"
                    response = success_message + " ~PK~" + str(server_public_key.decode('ascii'))
                    client_socket.send(response.encode())
                clients[client_socket] = User(username)
                clients[client_socket].public_key = client_public_key
...
if __name__ == "__main__":
    try:
        ...
        ADMIN_PASSWORD = decryption_cipher.encrypt(tmp_admin_password.encode())
        tmp_admin_password = ""
